...
@export var swing_hit_frame: int = 5
...
@export var enemy_health: float = 25.0
@export var knockback_resistance: float = 1.5
...
func _on_frame_changed() -> void:
	...
	var dist := global_position.distance_to(player.global_position)
	if dist <= swing_max_distance *1.5:
...
func take_damage(damage: int) -> void:
	...
	print("Zombie Big health: ", enemy_health)
	if enemy_health <= 0:
		die()
